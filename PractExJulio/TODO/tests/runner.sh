#!/bin/bash
# Ejecuta todos los tests y reporta diferencias
# Uso: ./runner.sh [--no-color]
ROOT=$(cd "$(dirname "$0")" && pwd)
INTERP="../interpreter.exe"
COLOR=1

if [ "$1" = "--no-color" ]; then
    COLOR=0
fi

TOTAL=0
PASS=0
FAIL=0

for test in "$ROOT"/*.p; do
    [ -f "$test" ] || continue
    name="$(basename "$test" .p)"
    expected="${test%.p}.expected"
    [ -f "$expected" ] || continue
    TOTAL=$((TOTAL + 1))
    input_file="${test%.p}.input"
    if [ -f "$input_file" ]; then
        output=$("$INTERP" "$test" < "$input_file" 2>&1 | sed 's/\x1b\[[0-9;]*m//g')
    else
        output=$("$INTERP" "$test" 2>&1 | sed 's/\x1b\[[0-9;]*m//g')
    fi
    expected_content=$(cat "$expected")
    if [ "$output" = "$expected_content" ]; then
        if [ $COLOR -eq 1 ]; then
            echo -e "\033[32mPASA\033[0m $name"
        else
            echo "PASA $name"
        fi
        PASS=$((PASS + 1))
    else
        if [ $COLOR -eq 1 ]; then
            echo -e "\033[31mFALLA\033[0m $name"
        else
            echo "FALLA $name"
        fi
        echo "  --- Esperado ($(echo "$expected_content" | wc -l) lineas) ---"
        echo "$expected_content"
        echo "  --- Obtenido ($(echo "$output" | wc -l) lineas) ---"
        echo "$output"
        echo "  ---"
        FAIL=$((FAIL + 1))
    fi
done

echo ""
echo "========================================="
if [ $COLOR -eq 1 ]; then
    echo -e "\033[36m$PASS/$TOTAL tests pasan, $FAIL fallan\033[0m"
else
    echo "$PASS/$TOTAL tests pasan, $FAIL fallan"
fi
exit $FAIL
