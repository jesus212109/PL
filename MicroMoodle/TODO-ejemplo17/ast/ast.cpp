...
void lp::DivEnteraNode::printAST()
{
  std::cout << "DivEnteraNode: /" << std::endl;
  std::cout << "\t"; 
	this->_left->printAST();
	std::cout << "\t"; 
	this->_right->printAST();
...
double lp::DivEnteraNode::evaluateNumber() 
{
	double result = 0.0;
	...
	if (this->getType() == NUMBER)
	{
    	...
    	if(std::abs(rightNumber) > ERROR_BOUND)
		{
				result = (long) leftNumber / (long)rightNumber;
		}
		else
		{
			warning("Runtime error", "Division by zero");
	...
	{
		warning("Runtime error: the expressions are not numeric for", "DivEntera");
	}
...
lp::DoWhileStmt::DoWhileStmt(BlockStmt *body, ExpNode *cond) {
    this->_body = body;
    this->_cond = cond;
}
...
lp::RepeatStmt::RepeatStmt(BlockStmt *body, ExpNode *cond) {
    this->_body = body;
    this->_cond = cond;
}
...
{
    // Obtener/crear la variable de control
    lp::Symbol *s = table.getSymbol(this->_var);
    ...
    if (s == NULL) {
		...
		var = (lp::NumericVariable *) s;
		var->setType(NUMBER);
	}

    double desde = this->_from->evaluateNumber();
    double hasta = this->_to->evaluateNumber();
    double paso = (this->_step != NULL) ? this->_step->evaluateNumber() : 1.0;

    if (paso == 0) {
        execerror("Runtime error: paso cero en bucle para", "");
    }
...
void lp::SwitchStmt::printAST(){
    ...
    std::cout << "\tExpression: ";
if (this->_switchExp != NULL)
    this->_switchExp->printAST();    std::cout << "\tCases:" << std::endl;
...
void lp::SwitchStmt::setSwitchExp(ExpNode *exp) {
    this->_switchExp = exp;
}
...
{
	// lugar does nothing in this implementation
	// (terminal cursor positioning would go here)
}
