...
%type <st> stmt asgn escribir leer si mientras block hacer_mientras repetir para selector case_list
...
/* NEW in example 17: SI, SI_NO, MIENTRAS */

%token ESCRIBIR LEER LEER_CADENA SI ENTONCES SI_NO FIN_SI
%token MIENTRAS HACER FIN_MIENTRAS
%token REPETIR HASTA HASTA_QUE
%token PARA DESDE PASO FIN_PARA
%token SELECCION CASO DEFECTO FIN_SELECCION
%token BORRAR_PANTALLA LUGAR
%token MOD DIV_ENTERA
%token CONCAT
...
%%
	...
	block
	: LETFCURLYBRACKET stmtlist RIGHTCURLYBRACKET
		{
	...
	SI controlSymbol cond ENTONCES stmtlist FIN_SI %prec THEN  SEMICOLON
		{
	...
	/*  NEW in example 17 */
	mientras
	: MIENTRAS controlSymbol cond HACER stmtlist FIN_MIENTRAS SEMICOLON
		{
	...
	hacer_mientras:
	 HACER controlSymbol stmtlist MIENTRAS controlSymbol cond SEMICOLON
		{
	// Create a new while statement node
	...
	repetir
	: REPETIR controlSymbol stmtlist HASTA_QUE cond SEMICOLON
		{
	...
	para
	: PARA controlSymbol VARIABLE DESDE exp HASTA exp HACER stmtlist FIN_PARA
		{
	...
	selector
	: SELECCION LPAREN exp RPAREN case_list FIN_SELECCION
		{
			lp::SwitchStmt *sw = (lp::SwitchStmt *) $5;
			sw->setSwitchExp($3);
			$$ = sw;
		}
	| SELECCION LPAREN exp RPAREN case_list DEFECTO ':' stmtlist FIN_SELECCION
		{
			lp::SwitchStmt *sw = (lp::SwitchStmt *) $5;
			sw->setSwitchExp($3);
	...
	| exp DIV_ENTERA exp
		{
	// Create a new DIV_ENTERA node
	$$ = new lp::DivEnteraNode($1, $3);
	}
