...
int lp::RelationalOperatorNode::getType()
{
	...
	if ( (this->_left->getType() == NUMBER) and (this->_right->getType() == NUMBER))
		...
		result = BOOL;
	else if ( (this->_left->getType() == STRING) && (this->_right->getType() == STRING))
		result = BOOL;
	else
...
/* ================================================================
 * NEW CODE - Added for the practice (Ejemplo 17 extensions)
 * ================================================================ */

/* NEW: StringNode implementation */
...
/* NEW: ConcatenationNode implementation */
...
double lp::ConcatenationNode::evaluateNumber()
{
    return 0.0;
}
...
{
    std::string left  = this->_left->evaluateString();
    std::string right = this->_right->evaluateString();
    return left + right;
}

/* NEW: IntegerDivisionNode implementation */
...
/* NEW: ForStmt evaluate */
void lp::ForStmt::evaluate()
{
    double fromVal  = this->_from->evaluateNumber();
    double toVal    = this->_to->evaluateNumber();
    double stepVal  = this->_step ? this->_step->evaluateNumber() : 1.0;
    int maxIter     = 100000;
    int iter        = 0;

    if (stepVal == 0.0) {
        execerror("Runtime error", "Zero step in for loop");
        return;
    }
    ...
    for (double i = fromVal;
         (stepVal > 0) ? (i <= toVal) : (i >= toVal);
         i += stepVal)
    {
        if (++iter > maxIter) {
            execerror("Runtime error", "Infinite loop detected in for");
            break;
        }
        ((lp::NumericVariable*)var)->setValue(i);
        this->_body->evaluate();
    }
}

/* NEW: RepeatStmt evaluate */
...
void lp::DoWhileStmt::evaluate()
{
    ...
    do {
        if (++iter > maxIter) {
            execerror("Runtime error", "Infinite loop detected in do-while");
            break;
        }
        this->_body->evaluate();
    } while (this->_cond->evaluateBool());
...
/* NEW: PostIncrementStmt evaluate */
...
/* NEW: CompoundAssignmentStmt evaluate (+:= and -:=) */
...
/* NEW: ClearScreenStmt evaluate */
void lp::ClearScreenStmt::evaluate()
{
    printf("\33[2J");
}

/* NEW: PlaceStmt evaluate */
...
/* NEW: SwitchStmt evaluate */
void lp::SwitchStmt::evaluate()
{
    double switchVal = this->_exp->evaluateNumber();
    bool matched = false;

    for (std::list<std::pair<lp::ExpNode*, lp::Statement*> >::iterator it = this->_cases.begin();
         it != this->_cases.end(); ++it) {
        double caseVal = it->first->evaluateNumber();
        if (!matched && std::abs(switchVal - caseVal) < 1.0e-6) {
            matched = true;
            it->second->evaluate();
        }
    }

    if (!matched && this->_default) {
        this->_default->evaluate();
    }
}

/* NEW: evaluate variable as string */
