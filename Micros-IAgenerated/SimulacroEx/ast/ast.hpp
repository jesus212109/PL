 ...
 class ExpNode 
{
  public:
	...
	*/
	...
	/*!	
		\brief   Evaluate the expression as STRING (NEW)
		\warning Virtual function: could be redefined in the heir classes
		\return  std::string
...
class VariableNode : public ExpNode 
{
	...
	*/
	...
	/*!	
		\brief   Evaluate the Variable as STRING (NEW)
		\return  std::string
		\sa		   getType, printAST, evaluateNumber
	*/
...
/* NEW: String literal node - cloned from NumberNode */
class StringNode : public ExpNode {
 private:
   std::string _value;
 public:
   StringNode(const std::string& value) { this->_value = value; }
   int getType();
   void printAST();
   double evaluateNumber() { return 0.0; }
   std::string evaluateString() { return this->_value; }
};
...
class ForStmt : public Statement {
 private:
  ...
  Statement *_body;
 public:
  ForStmt(std::string id, ExpNode *f, ExpNode *t, Statement *b, ExpNode *s = NULL)
    : _var(id), _from(f), _to(t), _body(b), _step(s) {}
  void printAST() { std::cout << "ForStmt" << std::endl; }
  void evaluate();
};
...
class DoWhileStmt : public Statement {
 private:
  ...
  ExpNode *_cond;
 public:
  DoWhileStmt(Statement *b, ExpNode *c) : _body(b), _cond(c) {}
  void printAST() { std::cout << "DoWhileStmt" << std::endl; }
  void evaluate();
};

/* NEW: Pre-increment statement (++x;) */
class PreIncrementStmt : public Statement {
 ...
 public:
  ...
  void printAST() { std::cout << "PreIncrementStmt: " << this->_var << std::endl; }
  void evaluate();
};

/* NEW: Post-increment statement (x++;) */
class PostIncrementStmt : public Statement {
 ...
 public:
  ...
  void printAST() { std::cout << "PostIncrementStmt: " << this->_var << std::endl; }
  void evaluate();
};

/* NEW: Compound assignment statement (+:= and -:=) */
class CompoundAssignmentStmt : public Statement {
 private:
  std::string _var;
  ExpNode *_exp;
  int _op;  /* PLUS or MINUS */
 public:
  CompoundAssignmentStmt(std::string id, ExpNode *e, int op) : _var(id), _exp(e), _op(op) {}
  void printAST() { std::cout << "CompoundAssignmentStmt: " << this->_var << std::endl; }
  void evaluate();
};

/* NEW: Clear screen statement */
class ClearScreenStmt : public Statement {
 public:
  void printAST() { std::cout << "ClearScreenStmt" << std::endl; }
  void evaluate();
};

/* NEW: Place cursor statement */
class PlaceStmt : public Statement {
 private:
  ExpNode *_x, *_y;
 public:
  PlaceStmt(ExpNode *x, ExpNode *y) : _x(x), _y(y) {}
  void printAST() { std::cout << "PlaceStmt" << std::endl; }
  void evaluate();
};
...
class SwitchStmt : public Statement {
 private:
  ExpNode *_exp;
  std::list<std::pair<ExpNode*, Statement*> > _cases;
  Statement *_default;
 public:
  SwitchStmt(ExpNode *e) : _exp(e), _default(NULL) {}
  void setSwitchExp(ExpNode *e) { _exp = e; }
  void addCase(ExpNode *val, Statement *body) {
    _cases.push_back(std::make_pair(val, body));
  }
  void setDefault(Statement *d) { _default = d; }
  void printAST() { std::cout << "SwitchStmt" << std::endl; }
  void evaluate();
};

/*!	
  \class TernaryNode
  \brief Nodo para el operador ternario (cond ? true : false)
